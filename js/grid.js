...
Grid.prototype.monotonicity2 = function() {
	...
	for (var y=0; y<4; y++) {
		...
		while ( next<4 ) {
			...
			if (next>=4) { next--; }
			var currentValue = this.cellOccupied({x:current, y:y}) ? Math.log(this.cellContent( this.indexes[current][y] ).value) / Math.log(2) : 0;
			var nextValue = this.cellOccupied({x:next, y:y}) ? Math.log(this.cellContent( this.indexes[next][y] ).value) / Math.log(2) : 0;
			if (currentValue > nextValue) {
...
Grid.prototype.isWin = function() {
	...
	for (var x=0; x<4; x++) {
		for (var y=0; y<4; y++) {
			if (self.cellOccupied(this.indexes[x][y]) && self.cellContent(this.indexes[x][y]).value == 2048) {
					return true;
			}
		}
	}
